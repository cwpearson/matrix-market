...
template<typename S>
S conj(S s) {return s;}
template<> std::complex<float> conj(std::complex<float> s) { return std::conj(s); }
template<> std::complex<double> conj(std::complex<double> s) { return std::conj(s); }
...
public:
    ...
    coo_type read_coo() {
        ...
        for (std::string line; std::getline(inf, line); ) {
            ...
            switch (info.scalar) {
                case Info::Scalar::unknown: throw std::logic_error("221");
                case Info::Scalar::PATTERN: entry.e = from_pattern<Scalar>(); break; // no more read
                case Info::Scalar::REAL: {
                    double re;
                    ss >> re;
                    if (0.0 == re) continue; // skip explicit 0
                ...
                case Info::Scalar::INTEGER: {
                    int64_t i;
                    ss >> i;
                    if (0 == i) continue; // skip explicit 0
                ...
                case Info::Scalar::COMPLEX: {
                    ...
                    ss >> real;
                    ss >> imag;
                    if (real == 0 && imag == 0) continue; // skip 0
                    entry.e = from_complex<Scalar>(std::complex<double>(real, imag));
                }
                default: throw std::logic_error("get_as_coo: only 'pattern', 'real', and 'integer' matrix market format supported");
            }
            ...
            switch(info.symmetry) {
                case Info::Symmetry::unknown: throw std::logic_error("251");
                case Info::Symmetry::GENERAL: break; // no-op
                ...
                default: throw std::logic_error("must be general, skew-symmetric or symmetric");
            }
